...
export default async function SuccessPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
    ...
    return (
        <div className="min-h-[80vh] flex items-center justify-center px-4">
            <div className="flex flex-col items-center gap-5 sm:gap-6 max-w-[420px] w-full text-center">
                ...
                <div className="flex flex-col gap-2">
                    <h1 className="text-[var(--text)] text-[20px] sm:text-[24px] font-bold leading-[133%]">
                ...
                <div className="flex flex-col gap-2 w-full bg-[#f9f9f9] rounded-[12px] p-4">
                    {["Payment verification", "Reserving your items", "Sending confirmation"].map((step, i) => (
                        <div key={i} className="flex items-center gap-3">
                            ...
                            <span className="text-[12px] sm:text-[13px] text-[#999]">{step}</span>
                        </div>
                    ))}
                </div>

                <p className="text-[11px] sm:text-[12px] text-[#bbb]">
