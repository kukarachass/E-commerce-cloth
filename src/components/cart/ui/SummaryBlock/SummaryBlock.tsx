"use client"

import PromoCodeInput from "@/components/cart/ui/PromoCodeInput";
import { formatPrice } from "@/lib/formatPrice";
import CustomsFee from "@/components/cart/ui/SummaryBlock/CustomsFee";
import ButtonPrimary from "@/components/ui/buttons/ButtonPrimary";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { CartWithConfig } from "@/types/cart";

interface SummaryBlockProps {
    cart: CartWithConfig;
}

const providers = [
    { path: "/card-prov/SVG.svg" },
    { path: "/card-prov/SVG-1.svg" },
    { path: "/card-prov/SVG-2.svg" },
    { path: "/card-prov/SVG-3.svg" },
    { path: "/card-prov/SVG-4.svg" },
]

export default function SummaryBlock({ cart }: SummaryBlockProps) {
    const router = useRouter();
    const canCheckout = cart.items.filter(i => i.productSize.stockAmount > 0);

    return (
        <div className="w-full lg:max-w-[400px] lg:sticky bg-[#f9f9f9] h-fit rounded-[10px] lg:top-[100px]">
            <div className="flex flex-col gap-4 p-5 sm:p-6">
                <h1 className="text-[var(--text)] text-[18px] sm:text-[20px] font-bold leading-[120%]">Order Summary</h1>
                <span className="text-[var(--text)] text-[15px] sm:text-[16px] pb-2">Apply promo code</span>
                <PromoCodeInput />
                <span className="underline text-[var(--text)] text-[14px] leading-[143%]">Promotions FAQ's</span>
            </div>
            <div className="h-[1px] w-full bg-[#f0f0f0]" />

            <div className="flex flex-col gap-2.5 p-5 sm:p-6 w-full">
                <div className="flex flex-row justify-between text-[var(--text)] text-[14px] font-bold leading-[143%]">
                    <h3>Total Items({cart.items.length})</h3>
                    <span>{formatPrice(Number(cart.totalAmount))}</span>
                </div>
                <div className="flex flex-row justify-between text-[var(--text)] text-[14px] leading-[143%] gap-2">
                    <span className="shrink-0">Shipping Fee</span>
                    {cart.isShippingFree ? (
                        <span className="text-[var(--text)] text-[14px] text-right">You've unlocked free shipping</span>
                    ) : (
                        <span>{formatPrice(Number(cart.shippingFee))}</span>
                    )}
                </div>
                <CustomsFee customsFee={cart.customsFee} />
            </div>

            <div className="h-[1px] w-full bg-[#f0f0f0]" />
            <div className="flex flex-col gap-2.5 p-5 sm:p-6 w-full">
                <div className="flex flex-row justify-between text-[var(--text)] font-bold text-[18px] sm:text-[20px] leading-[120%]">
                    <h3>Total</h3>
                    <span>{formatPrice(Number(cart.grandTotal))}</span>
                </div>
                <div className="flex flex-row justify-between text-[var(--text)] font-bold text-[14px] leading-[143%]">
                    <h3>Total saved</h3>
                    <span>{formatPrice(cart.totalSaved)}</span>
                </div>
                <span className="text-[var(--text)] text-[12px] leading-[133%]">
                    This is compared to the recommended retail price. This is the price set by the manufacturer, converted into GBP where applicable.
                </span>
            </div>

            <div className="flex flex-col gap-2 p-5 sm:p-6 w-full">
                <ButtonPrimary
                    disabled={canCheckout.length <= 0}
                    onClick={() => router.push("/checkout?step=1")}
                    className="w-full"
                    variant={"primary"}
                >
                    Continue to checkout
                </ButtonPrimary>
                <ButtonPrimary
                    className="w-full"
                    onClick={() => router.back()}
                    variant={"secondary"}
                >
                    Continue shopping
                </ButtonPrimary>
            </div>

            <div className="flex flex-row flex-wrap items-center justify-center gap-2 pb-6 px-4">
                {providers.map(prov => (
                    <Image key={prov.path} src={prov.path} alt={prov.path} width={46} height={24} />
                ))}
            </div>
        </div>
    )
}