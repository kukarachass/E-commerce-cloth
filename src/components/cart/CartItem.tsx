...
export default function CartItem({cartItem}: any) {
    return (
            ...
            <div className="flex flex-row justify-between">
                <div className="flex flex-row gap-4 items-center">
                    <div className="w-[80px] h-[120px] relative">
                        <Image src={cartItem.imageUrl} alt={cartItem.name} fill/>
                    </div>
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col">
                            <span className="text-[var(--text)] font-bold text-[16px] leading-[150%]">{cartItem.name}</span>
                            <span className="text-[var(--text)] text-[14px] leading-[143%]">{cartItem.desc}</span>
                        </div>
                        <div className="flex flex-row gap-1 text-[14px]">
                                <span className="text-[var(--text)]">Size</span>
                                <span className="font-bold text-[var(--text)]">{cartItem.size}</span>
