        ...
        <>
            ...
            <div className="max-w-[640px] mx-auto py-10 sm:py-16 px-4 flex flex-col gap-8 sm:gap-10">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <h1 className="text-[var(--text)] font-bold text-[26px] sm:text-[32px] leading-[120%]">
                        Order confirmed
                    </h1>
                    <p className="text-[#999] text-[14px] sm:text-[15px] leading-[160%]">
                        Thanks for your purchase — great taste. We're on it.
                    </p>
                </div>

                <div className="flex flex-col gap-1 p-4 sm:p-5 bg-[#f9f9f9] rounded-[12px]">
                    <span className="text-[11px] sm:text-[12px] text-[#999] uppercase tracking-wider">Order number</span>
                    <span className="text-[var(--text)] font-semibold text-[14px] sm:text-[15px] font-mono">
                ...
                <div className="flex flex-col gap-0">
                    {steps.map((step, i) => (
                        <div key={i} className="flex flex-row gap-3 sm:gap-4">
                            ...
                            <div className={`flex flex-col gap-1 min-w-0 ${i < steps.length - 1 ? "pb-6" : ""}`}>
                                <span className="text-[var(--text)] font-semibold text-[13px] sm:text-[14px]">{step.title}</span>
                                <p className="text-[#999] text-[12px] sm:text-[13px] leading-[160%]">{step.description}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="flex flex-row items-center gap-3 p-4 border border-[#ebebeb] rounded-[12px]">
                    ...
                    <span className="text-[12px] sm:text-[13px] text-[#999]">
                        Shipping: <span className="font-semibold text-[var(--text)]">4–8 working days</span>
                    </span>
                </div>
