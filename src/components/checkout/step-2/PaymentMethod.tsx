...
export default function PaymentMethod() {
    ...
    return (
        <div className="flex flex-col gap-4 w-full">
            <h2 className="font-bold text-[24px]">Payment method</h2>
                    ...
                    <div key={m.id} className="flex flex-col gap-4">
                        <div className="flex flex-row justify-between">
                            ...
                            >
                                ...
                                {/* иконка */}
                                <div className="w-[40px] h-[30px] relative rounded-lg overflow-hidden">
                                    <Image src={m.icon} alt={m.label} fill className="object-cover" />
                                </div>

                                <span className="text-[16px] text-[var(--text)] font-medium">{m.label}</span>
                            </button>
                            {m.id === "card" && (
                                <div className="flex flex-row items-center gap-2">
                                    {cardProviders.map(prov => (
                                        <Image key={prov.icon} src={prov.icon} alt={prov.icon} width={32} height={24}/>
                                    ))}
                                </div>
                            )}
                        </div>
